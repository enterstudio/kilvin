...
class Members
{
    ...
    */
    public function listMembers($message = '')
    {
        ...
        $right_links[] = [
            BASE.'?C=Members'.AMP.'M=memberSearch',
        ...
        $r .= Cp::formOpen(array('action' => 'C=Members'.AMP.'M=listMembers'));
        ...
        $pageurl = BASE.'?C=Members'.AMP.'M=listMembers';
        ...
        $r .= Cp::formOpen(
            [
                'action'    => 'C=Members'.AMP.'M=deleteMemberConfirm',
                'name'      => 'target',
                'id'        => 'target'
            ]
        );
    ...
    */
    public function deleteMemberConfirm()
    {
        ...
        $r = Cp::formOpen(['action' => 'C=Members'.AMP.'M=deleteMembers']);
        ...
        $r .= Cp::quickDiv('littlePadding', Cp::quickDiv('alert', __('cp.action_can_not_be_undone')));
    ...
    */
    public function loginAsMember()
    {
        ...
        $r  = Cp::formOpen(
            array('action' => 'C=Members'.AMP.'M=doLoginAsMember'),
            array('mid' => $id)
        );
    ...
    * @return string
    */
    public function memberGroupManager($message = '')
    {
        $row_limit = 20;
        $paginate = '';
        ...
        if ($count > $row_limit)
        {
            ...
            $paginate = Cp::pager(  BASE.'?C=Members'.AMP.'M=memberGroupManager',
                                      $count,
                                      $row_limit,
                                      $row_count,
                                      'row'
        ...
        $right_links[] = [
            BASE.'?C=Members'.AMP.'M=editMemberGroup',
        ...
        foreach($query as $row)
        {
            ...
            Cp::$body .= Cp::tableCell('', Cp::anchor(BASE.'?C=Members'.AMP.'M=editMemberGroup'.AMP.'group_id='.$row->group_id, __('cp.edit')), '20%');
            ...
            $delete = ( ! in_array($row->group_id, $this->no_delete)) ?
                Cp::anchor(
                    BASE.'?C=Members'.
                        AMP.'M=deleteMemberGroupConfirm'.
        ...
        Cp::$body .= Cp::formOpen(['action' => 'C=Members'.AMP.'M=edit_mbr_group']);
    ...
    *
    * @param string $msg
    * @param integer $group_id When you are coming from another function loading this one
    * @return string
    */
    public function editMemberGroup($msg='', $group_id = null)
    {
        // ------------------------------------
        ...
        $r  = Cp::formOpen(
            [
                'action' => 'C=Members'.AMP.'M=updateMemberGroup'
            ]
        );
        ...
        if ($id != 1)
        {
            foreach($query as $row)
            {
                ...
                $weblog_names[$field] = $row->weblog_title;
        ...
        if ($id != 1)
        {
            foreach(Plugins::list() as $plugin)
            {
                ...
                $plugin_names[$field] = $plugin->plugin_name;
                $group_data[$field] = $status;
            }
        }
        ...
        foreach ($G as $g_key => $g_val)
        {
            ...
            foreach($g_val as $key => $val)
            {
                ...
                {
                    $line = __('members.can_post_in').Cp::quickSpan('alert', $weblog_names[$key]);
                }
        ...
        else
        {
            $r .= Cp::quickDiv(
                'paddingTop',
                Cp::input_submit(__('cp.update')).
                NBS.
                Cp::input_submit(__('cp.update_and_return'), 'return')
            );
        }
    ...
    */
    public function updateMemberGroup()
    {
        ...
        if ($edit === false)
        {
            ...
            $message = __('members.member_group_created').' '.Request::input('group_name');
        }
        else
        {
            ...
            $message = __('members.member_group_updated').'&nbsp;'.Request::input('group_name');
        }
        ...
        if ($return == true) {
            return $this->memberGroupManager($message);
        }
    ...
    */
    public function deleteMemberGroupConfirm()
    {
        ...
        Cp::$crumb = Cp::anchor(BASE.'?C=Administration'.AMP.'area=members_and_groups', __('admin.members_and_groups')).
                      Cp::breadcrumbItem(Cp::anchor(BASE.'?C=Members'.AMP.'M=group_manager', __('admin.member_groups'))).
                      Cp::breadcrumbItem(__('members.delete_member_group'));

        Cp::$body = Cp::formOpen(['action' => 'C=Members'.AMP.'M=deleteMemberGroup'.AMP.'group_id='.$group_id]).
        ...
        Cp::$body .= Cp::heading(Cp::quickSpan('alert', __('members.delete_member_group')))
                     ...
                     .Cp::quickDiv('alert', BR.__('cp.action_can_not_be_undone').BR.BR);
    ...
    * @return string
    */
    public function deleteMember_group()
    {
        // ------------------------------------
        //  Only super admins can delete member groups
        // ------------------------------------
        ...
        return $this->memberGroupManager(__('members.member_group_deleted'));
    }
    ...
    */
    public function registerMember()
    {
        ...
        // Build the output
        $r  = Cp::formOpen(['action' => 'C=Members'.AMP.'M=createMember']);
    ...
    */
    public function createMember()
    {
        ...
        $message = __('account.new_member_added');
    ...
    */
    public function memberBanning()
    {
        ...
        $r  = Cp::formOpen(array('action' => 'C=Members'.AMP.'M=saveBanningPreferences')).
              Cp::quickDiv('tableHeading', __('members.user_banning'));

        if (Request::input('U'))
        {
            $r .= Cp::quickDiv('success-message', __('members.ban_preferences_updated'));
        }
        ...
        $r .=   Cp::div().BR.
                Cp::input_submit(__('cp.update')).BR.BR.BR.
                '</div>'.PHP_EOL.
                '</div>'.PHP_EOL;
    ...
    */
    public function saveBanningPreferences()
    {
        ...
        return redirect('?C=Members&M=memberBanning&U=1');
    }
    ...
    * @return string
    */
    public function profileFields($group_id = '')
    {
        if ( ! Session::access('can_admin_members')) {
            return Cp::unauthorizedAccess();
        }

        Cp::$title  = __('members.member_profile_fields');
        Cp::$crumb  = Cp::anchor(BASE.'?C=Administration'.AMP.'area=members_and_groups', __('admin.members_and_groups')).
            Cp::breadcrumbItem(__('members.member_profile_fields'));

        $right_links[] = [
            BASE.'?C=Members'.AMP.'M=editProfileField',
            __('members.create_new_profile_field')
        ];

        $r  = Cp::header(__('members.member_profile_fields'), $right_links);

        // Build the output
        if (Request::input('U')) {
            $r .= Cp::quickDiv('success-message', __('members.field_updated'));
        }

        $query = DB::table('member_fields')
            ->select('member_field_id', 'field_order', 'field_label')
            ->orderBy('field_order')
            ->get();
        ...
        {
            Cp::$body  = Cp::div('box');
            Cp::$body .= Cp::quickDiv('littlePadding', Cp::heading(__('members.no_member_profile_fields'), 5));
                Cp::$body .= Cp::quickDiv('littlePadding', Cp::anchor(BASE.'?C=Members'.AMP.'M=editProfileField', __('members.create_new_profile_field')));
            Cp::$body .= '</div>'.PHP_EOL;
        ...
        {
            $r .= '<tr>'.PHP_EOL;
            $r .= Cp::tableCell('', Cp::quickSpan('defaultBold', $row->field_label), '40%');
            $r .= Cp::tableCell('', Cp::anchor(BASE.'?C=Members'.AMP.'M=editProfileField'.AMP.'member_field_id='.$row->member_field_id, __('cp.edit')), '30%');
            $r .= Cp::tableCell('', Cp::anchor(BASE.'?C=Members'.AMP.'M=deleteFieldConfirm'.AMP.'member_field_id='.$row->member_field_id, __('cp.delete')), '30%');
            $r .= '</tr>'.PHP_EOL;
        }

        $r .= '</table>'.PHP_EOL;

        $r .= Cp::quickDiv('paddedWrapper', Cp::anchor(BASE.'?C=Members'.AMP.'M=editFieldOrder', __('members.edit_field_order')));
    ...
    */
    public function editProfileField()
    {
        ...
        $type = ($member_field_id = Request::input('member_field_id')) ? 'edit' : 'new';
        ...
        $query = DB::table('member_fields')
            ->where('member_field_id', $member_field_id)
            ->first();
...
EOT;
        ...
        $r .= Cp::formOpen(array('action' => 'C=Members'.AMP.'M=updateProfileField'.AMP.'U=1'));
        $r .= Cp::input_hidden('member_field_id', $member_field_id);
        $r .= Cp::input_hidden('cur_field_name', $field_name);
        ...
        $r .= Cp::tableCell('', Cp::input_text('field_label', $field_label, '50', '60', 'input', '300px'), '60%');
        $r .= '</tr>'.PHP_EOL;
        ...
        $r .= '<tr>'.PHP_EOL;
        $r .= Cp::tableCell('', Cp::quickSpan('defaultBold', Cp::required().NBS.__('members.field_handle')).Cp::quickDiv('littlePadding', __('members.fieldname_cont')), '40%');
        ...
        $typopts .= Cp::quickDiv('defaultBold', __('members.max_length')).Cp::quickDiv('littlePadding', Cp::input_text('field_maxlength', $field_maxlength, '4', '3', 'input', '30px'));
        $typopts .= '</div>'.PHP_EOL;
        ...
        $typopts .= Cp::quickDiv('defaultBold', __('members.text_area_rows')).Cp::quickDiv('littlePadding', Cp::input_text('textarea_num_rows', $textarea_num_rows, '4', '3', 'input', '30px'));
        $typopts .= '</div>'.PHP_EOL;
        ...
        $typopts .= Cp::quickDiv('defaultBold', __('members.pull_down_items')).Cp::quickDiv('default', __('admin.field_list_instructions')).Cp::input_textarea('field_list_items', $field_list_items, 10, 'textarea', '400px');
        $typopts .= '</div>'.PHP_EOL;
        ...
        if ($is_field_required == '') $is_field_required = 'n';

        $r .= '<tr>'.PHP_EOL;
        $r .= Cp::tableCell('', Cp::quickSpan('defaultBold', __('admin.is_field_required')), '40%');
        $r .= Cp::tableCell('', __('cp.yes').'&nbsp;'.Cp::input_radio('is_field_required', 'y', ($is_field_required == 'y') ? 1 : '').'&nbsp;'.__('cp.no').'&nbsp;'.Cp::input_radio('is_field_required', 'n', ($is_field_required == 'n') ? 1 : ''), '60%');
        $r .= '</tr>'.PHP_EOL;
        ...
        if ($is_field_public == '') $is_field_public = 'y';
        ...
        $r .= Cp::tableCell('', __('cp.yes').'&nbsp;'.Cp::input_radio('is_field_public', 'y', ($is_field_public == 'y') ? 1 : '').'&nbsp;'.__('cp.no').'&nbsp;'.Cp::input_radio('is_field_public', 'n', ($is_field_public == 'n') ? 1 : ''), '60%');
        $r .= '</tr>'.PHP_EOL;
        ...
        Cp::$crumb = Cp::anchor(BASE.'?C=Administration'.AMP.'area=members_and_groups', __('admin.members_and_groups')).
                      Cp::breadcrumbItem(Cp::anchor(BASE.'?C=Members'.AMP.'M=profileFields', __('members.member_profile_fields'))).
    ...
    */
    public function updateProfileField()
    {
        ...
        if ($field_count > 0) {
            ...
            if ($edit === true && $input['field_name'] != $input['cur_field_name']) {
                $errors[] = __('members.duplicate_field_name');
            }
        }
        ...
        if ($edit === true) {
            ...
            if ($input['cur_field_name'] !== $input['field_name']) {

                Schema::table('member_data', function ($table) use ($input) {
                    $table->renameColumn('m_field_'.$input['cur_field_name'], 'm_field_'.$input['field_name']);
                });
            }
            ...
            {
                if ($f_type == 'string') {
                    $table->string('m_field_'.$input['field_name'], $n)->change();
                } else {
                    $table->text('m_field_'.$input['field_name'])->change();
                }
            });
            ...
            DB::table('member_fields')
                ->where('member_field_id', $input['member_field_id'])
                ->update($input);
        }
        ...
        if ($edit === false) {
            ...
            {
                if ($f_type == 'string') {
                    $table->string('m_field_'.$input['field_name'], $n);
                } else {
                    $table->text('m_field_'.$input['field_name']);
                }
            });
        }
    ...
    */
    public function deleteFieldConfirm()
    {
        ...
        Cp::$body =
            Cp::formOpen([
                'action' => 'C=Members'.AMP.'M=deleteProfileField'.AMP.'member_field_id='.$member_field_id
            ]).
                Cp::input_hidden('member_field_id', $member_field_id).
                Cp::quickDiv('alertHeading', __('admin.delete_field')).
                Cp::div('box').
                    Cp::quickDiv('littlePadding', '<b>'.__('admin.delete_field_confirmation').'</b>').
                    Cp::quickDiv('littlePadding', '<i>'.$query->field_label.'</i>').
                    Cp::quickDiv('alert', BR.__('cp.action_can_not_be_undone')).
    ...
    */
    public function deleteProfileField()
    {
        ...
        $query = DB::table('member_fields')
            ->where('member_field_id', $member_field_id)
            ->select('field_name', 'field_label', 'member_field_id')
            ->first();
        ...
        {
            $table->dropColumn('m_field_'.$query->field_name);
        });

        DB::table('member_fields')->where('member_field_id', $query->member_field_id)->delete();

        Cp::log(__('members.profile_field_deleted').'&nbsp;'.$query->field_label);
    ...
    */
    public function editFieldOrder()
    {
        ...
        $query = DB::table('member_fields')
            ->orderBy('field_order')
            ->select('field_label', 'field_name', 'field_order')
            ->get();
        ...
        $r  = Cp::formOpen(['action' => 'C=Members'.AMP.'M=updateFieldOrder']);
        $r .= Cp::table('tableBorder', '0', '10', '100%');
        ...
        {
            $r .= '<tr>'.PHP_EOL;
            $r .= Cp::tableCell('', $row->field_label);
            $r .= Cp::tableCell('', Cp::input_text($row->field_name, $row->field_order, '4', '3', 'input', '30px'));
            $r .= '</tr>'.PHP_EOL;
        }
    ...
    public function updateFieldOrder()
    {
        ...
        foreach ($query as $row)
        {
            DB::table('member_fields')
                ...
                ->update(['field_order' => Request::input($row->field_name)]);
        }
    ...
    */
    ...
    {
        Cp::$body  = Cp::formOpen(array('action' => 'C=Members'.AMP.'M=doMemberSearch'));
    ...
    */
    ...
    {
        $pageurl = BASE.'?C=Members'.AMP.'M=doMemberSearch';
        ...
        if (Request::input('criteria')) {
            if (!Request::input('keywords') && ! Request::input('group_id')) {
                return redirect('?');
            }

            if (substr(Request::input('criteria'), 0, 11) == 'm_field_') {
        ...
        if ($Q = Request::input('Q')) {
            ...
            if (empty($search) && $custom === false && !Request::input('group_id')) {
                return redirect('?C=Members&M=memberSearch');
            }
            ...
            if (Request::input('group_id') && Request::input('group_id') != 'any') {
                $search_query->where('member_groups.group_id', Request::input('group_id'));
            }
        ...
        if ($custom !== false) {
            ...
            if (Request::input('exact_match')) {
                $query->where('member_data.m_field_'.$custom[0], $custom[1]);
            } else {
                $query->where('member_data.m_field_'.$custom[0], 'LIKE', '%'.$custom[1].'%');
            }
        }
        ...
        if ($total_count == 0)  {
            return $this->memberSearch(Cp::quickDiv('littlePadding', Cp::quickDiv('alert', __('members.no_search_results'))));
        }
        ...
        $r .= Cp::table('tableBorder', '0', '', '100%').
              ...
              Cp::tableCell('tableHeadingAlt', __('account.screen_name')).
              Cp::tableCell('tableHeadingAlt', __('account.email')).
              Cp::tableCell('tableHeadingAlt', __('cp.ip_address')).
              Cp::tableCell('tableHeadingAlt', __('account.join_date')).
              Cp::tableCell('tableHeadingAlt', __('admin.member_group')).
              Cp::tableCell('tableHeadingAlt', Cp::input_checkbox('toggle_all')).
              '</tr>'.PHP_EOL;
