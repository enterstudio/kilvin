...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Exception $e)
    {
        ...
        if ($e instanceof \Illuminate\Database\QueryException) {
            if (preg_match('/([A-Z]+) command denied to user/i', $e->getMessage(), $match)) {
                ...
                if (REQUEST === 'CP') {
                    ...
                    return response()->view('cp.errors.error', $vars, 400);
                }
        ...
        if ($e instanceof CmsFatalException){
            return response()->view('fatal-error', ['message' => $e->getMessage()], 500);
        }
        ...
        if ($e instanceof CmsCpPageNotFound) {
           $vars = [
                ...
                'link'   => ['url' => 'JavaScript:history.go(-1)', 'name' => __('core.go_back')]
            ];

            return response()->view('cp.errors.error', $vars, 404);
        }
    ...
    protected function sendCsrfErrorPage()
    {
        if (REQUEST === 'CP') {
            ...
            return response()->view('cp.errors.error', $vars, 500);
        }
